...
driver.get('https://tuoitre.vn/nha-dat/trang-20.htm')
...
elems = driver.find_elements(By.CSS_SELECTOR, '.box-category .box-content-title .box-title-text a')
...
for index, row in df.head(max_rows).iterrows():
    ...
    try:
        ...
        date_div = driver.find_element(By.CSS_SELECTOR, ".detail-time div[data-role='publishdate']")
        df.at[index, 'Ngày'] = date_div.text.strip() if date_div else 'N/A'
        print(date_div)

        # Get detailed content from the page
        paragraphs  = driver.find_elements(By.CSS_SELECTOR, '.detail-cmain p')
        detailed_content = ""
...
excel_file_path = f'{current_date}_tuoitre_with_content.xlsx'
df.to_excel(excel_file_path, index=False, engine='openpyxl')

print(f"Data has been saved to {excel_file_path}")

# py cr_content_tuoitre.py
