...
driver.get('https://baodautu.vn/batdongsan/')
...
elems = driver.find_elements(By.CSS_SELECTOR, '.list_news_home article .desc_list_news_home .fs22')
...
for index, row in df.head(max_rows).iterrows():
    ...
    try:
        elements_with_tags = driver.find_elements(By.CSS_SELECTOR, '.tag_detail a')
        ...
        date_div = driver.find_element(By.CSS_SELECTOR, ".post-time")
        df.at[index, 'Ngày'] = date_div.text.strip() if date_div else 'N/A'


        # Get detailed content from the page
        paragraphs  = driver.find_elements(By.CSS_SELECTOR, '#content_detail_news p')
        detailed_content = ""
...
excel_file_path = f'{current_date}_baodautu_with_content.xlsx'
df.to_excel(excel_file_path, index=False, engine='openpyxl')

print(f"Data has been saved to {excel_file_path}")

# py cr_content_baodautu.py
